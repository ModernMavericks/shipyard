...
#
# The merge logic is arch-agnostic, so this needs any two architectures the box can compile -- but
# WHICH two is not a free choice, and getting it wrong cost this suite its coverage where it matters
# most. It used to require i386, which only a 10.9 box can build, so on macos-26 it exited 77 and the
# script that assembles BOTH shipped artifacts (the universal CMake and the universal updater) ran
# untested on the only machine that ever runs it for real.
#
# So: x86_64 plus whichever second arch this box has. i386 on 10.9, arm64 on Apple Silicon -- exactly
# one of the two works on each, and the suite is the same either way.
set -eu
...
# Identical thin x86_64 is refused when --require-archs demands both arches.
...
# The two-arch merge with --require-archs passes.
...
# x86_64h with architecture $ALT+x86_64 must NOT satisfy --require-archs "$ALT x86_64h".
