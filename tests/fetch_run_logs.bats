...
# from $T/jobs.txt ("<id> <status>" per line) and each log from $T/logs/<id>.
setup() {
...
@test "a log it cannot fetch is an error, naming the permission it needs" {
  printf '101 completed\n' > "$T/jobs.txt"      # and no $T/logs/101
  run --separate-stderr fetch
  [ "$status" -eq 1 ]
  [[ "$stderr" == *"actions: read"* ]] || false
}
